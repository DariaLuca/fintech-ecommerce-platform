...
import { ShoppingCart, Menu, X, Landmark, User, LogOut } from 'lucide-react';
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between h-20 items-center">
                    ...
                    <div className="flex items-center space-x-4 lg:space-x-6">
                        {isLoading ? (
                            <div className="w-9 h-9 animate-pulse bg-card-bg rounded-full"></div>
                        ) : user ? (
                            <button onClick={() => supabase.auth.signOut()} className="p-2 text-foreground hover:text-red-500 transition-colors rounded-full hover:bg-card-bg" title="Sign Out">
                                <LogOut size={20} />
                            </button>
                        ) : (
                            <Link href="/login" className="p-2 text-foreground hover:text-blue-600 transition-colors rounded-full hover:bg-card-bg" title="Sign In Space">
                                <User size={20} />
                            </Link>
                        )}
                        ...
                        <div className="lg:hidden flex items-center">
                            ...
                            >
                                {isMobileMenuOpen ? <X size={28} className="text-foreground" /> : <Menu size={28} className="text-foreground" />}
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="px-4 pt-2 pb-6 space-y-1">
                        {categories.map((category) => (
                            <button
                                ...
                                className={`block w-full text-left px-3 py-3 rounded-xl text-base font-medium transition-colors ${activeTab === category ? 'text-blue-600 bg-blue-500/10' : 'text-foreground hover:text-blue-600 hover:bg-card-border'}`}
                            >
                                {category}
                            </button>
                        ))}
